...
namespace Reach.DAL
{
    public class ReachInitializer : DropCreateDatabaseIfModelChanges<ReachContext>
    {
        protected override void Seed(ReachContext context)
        {
            ...
            {
                item.BigThumbnail = handler.GetBigThumbnailByYoukuId(item.YoukuId);
                item.Thumbnail = handler.GetThumbnailByYoukuId(item.YoukuId);
                item.Url = handler.GetVideoUrlByYoukuId(item.YoukuId);
            }
